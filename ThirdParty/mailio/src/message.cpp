...
namespace mailio
{
    ...
    */
    mailboxes message::parse_address_list(const string & al) const
    {
        string address_list = al;
        ...
        for (auto ch = address_list.begin(); ch != address_list.end(); ch++)
        {
            ...
            switch (state)
            {
                case state_t::BEGIN:
                {
                    if (isspace(cc))
                        ;
                    else if (isalpha(cc) || isdigit(cc) || ATEXT.find(cc) != string::npos)
                    {
                        token += cc;
                    ...
                    else if (cc == codec::LESS_THAN_CHAR)
                        state = state_t::ADDRBRBEG;
                    else
                        throw message_error("Parsing failure of address or group at `" + string(1, cc) + "`.");
                    ...
                    if (ch == address_list.end() - 1)
                    {
                        ...
                        {
                            if (group_found)
                                throw message_error("Parsing failure of group at `" + string(1, cc) + "`.");
                            else
                            {
                                if (!token.empty())
                                {
                        ...
                        // `QNAMEADDRBEG` or `ADDRBRBEG`
                            ...
                            throw message_error("Parsing failure of name or address at `" + string(1, cc) + "`.");
                    }
                ...
                {
                    if (isalpha(cc) || isdigit(cc) || ATEXT.find(cc) != string::npos)
                        token += cc;
                    else if (cc == codec::MONKEY_CHAR)
                    {
                        token += cc;
                        state = state_t::ADDR;
                        monkey_found = true;
                    ...
                    if (ch == address_list.end() - 1)
                    {
                        if (state == state_t::NAMEADDRGRP)
                        {
                            ...
                            throw message_error("Parsing failure of address at `" + string(1, cc) + "`.");
                        else if (state == state_t::NAME)
                            throw message_error("Parsing failure of name at `" + string(1, cc) + "` .");
                ...
                case state_t::NAME:
                {
                    if (isalpha(cc) || isdigit(cc) || ATEXT.find(cc) != string::npos || isspace(cc))
                        token += cc;
                    else if (cc == codec::LESS_THAN_CHAR)
                    {
                        cur_address.name = token;
                ...
                case state_t::ADDR:
                {
                    ...
                    else if (cc == codec::LEFT_PARENTHESIS_CHAR)
                    {
                        if (group_found)
                            throw message_error("Parsing failure of group at `" + string(1, cc) + "`.");
                        else
                    ...
                    if (ch == address_list.end() - 1)
                    {
                        ...
                        else if (state == state_t::BEGIN)
                        {
                            ...
                            throw message_error("Parsing failure of comment at `" + string(1, cc) + "`.");
                    }
                ...
                {
                    if (isalpha(cc) || isdigit(cc) || isspace(cc) || QTEXT.find(cc) != string::npos)
                        token += cc;
                    // backslash is invisible, see [rfc 5322, section 3.2.4]
                    else if (cc == codec::BACKSLASH_CHAR)
                        ;
                    else if (cc == codec::QUOTE_CHAR)
                    {
                        cur_address.name = token;
                ...
                case state_t::ADDRBRBEG:
                {
                    ...
                    else if (cc == codec::GREATER_THAN_CHAR)
                    {
                        cur_address.address = token;
                        token.clear();
                        mail_addrs.push_back(cur_address);
                ...
                case state_t::COMMBEG:
                {
                    if (isalpha(cc) || isdigit(cc) || ATEXT.find(cc) != string::npos || isspace(cc))
                        ;
                    else if (cc == codec::RIGHT_PARENTHESIS_CHAR)
